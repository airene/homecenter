...
const modifyDNS = async (ip = null) => {
    try {
        const timestamp = parseInt(String(new Date().getTime() / 1000))
        const date = getDate(timestamp)
        const payloadJson = { Domain: conf.ddns.domain, RecordType: 'A', RecordLine: '默认', Value: ip ?? (await getWanIP()), RecordId: conf.ddns.recordId, SubDomain: conf.ddns.subDomain }
